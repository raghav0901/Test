...
import datetime  # ADDED: for timestamping startup message
import logging   # ADDED: optional server logging
...
# ------------------- ADDED: create a startup message -----------------------
startup_message = (
    f"App started at {datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')}. "
    f"Loaded {full_df.shape[0]} rows."
)
# Log to server console
logging.info(startup_message)
# Also make message available to Flask for the first-request hook
server.startup_message = startup_message
# -------------------------------------------------------------------------
...
# -------------------------------------------------------------------------
# Layout: include a startup banner (dismissible) + store
# -------------------------------------------------------------------------
app.layout = dbc.Container([
    # Store the startup message so it's available client-side if needed
    dcc.Store(id='startup-message-store', data=startup_message),  # ADDED

    # Startup banner (shows on initial page load; dismissible)
    dbc.Row([
        dbc.Col(
            dbc.Alert(startup_message, id='startup-alert', color='info', is_open=True, dismissable=True),
            width=12
        )
    ], className="mb-3"),
...
# -------------------------------------------------------------------------
# Optional: print/log exactly once when the first HTTP request arrives
# (Useful when you run via Gunicorn / Flask -- prints on first request)
# -------------------------------------------------------------------------
@server.before_first_request
def announce_start():
    # server.startup_message was set earlier
    msg = getattr(server, 'startup_message', 'App started (no message set)')
    print("Flask before_first_request:", msg)
    logging.info("Flask before_first_request: " + msg)
...
# (mito_callback commented-out section unchanged)
